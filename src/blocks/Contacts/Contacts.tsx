...
import { saveContactForm } from '@/app/save-contact-form'
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12 max-w-7xl mx-auto">
          ...
          <div className="lg:col-span-5 space-y-5">
            ...
            {[
              ...
              <div key={idx}
                   className="group flex gap-5 border border-border/60 bg-white/80 backdrop-blur-sm rounded-[2rem] p-6 items-start shadow-card-soft  ">
                <div
                  className={`w-14 h-14 rounded-2xl flex items-center justify-center shrink-0 ${item.color} shadow-sm text-white `}>
                  <item.icon size={26} />
                </div>
                <div className="flex flex-col">
                  ...
                  {item.desc &&
                    <p className="text-xs italic text-muted-foreground mt-2 leading-relaxed">{item.desc}</p>}
                </div>
              </div>
            ))}
            ...
            <div className="border border-border/60 bg-white/80 backdrop-blur-sm rounded-[2rem] p-6 shadow-card-soft">
              <div className="flex gap-5 items-center mb-4">
                <div
                  className="w-14 h-14 rounded-2xl flex items-center justify-center shrink-0 bg-hero-violet shadow-sm text-white">
                  <Clock size={26} />
                </div>
              ...
              <div className="space-y-2 px-2">
                {openingHours?.length > 0 ? (
                  openingHours.map((item: { dayRange: string, hours: string }, i: number) => (
                    <div key={i}
                         className="flex justify-between text-sm py-2 border-b border-dashed border-border last:border-0">
            ...
            <div className="bg-white border-2 border-primary/5 p-8 md:p-12 rounded-[3rem] shadow-glow relative">
              ...
              <form className="grid grid-cols-1 md:grid-cols-2 gap-6"
                ...
                <div className="flex flex-col gap-2">
                  <label className="font-bold text-sm ml-2 text-foreground/70 uppercase tracking-wider">Nome e
                    cognome*</label>
                  <input name="name" type="text" required
                         className="bg-muted/50 p-4 rounded-2xl border-2 border-transparent focus:border-primary/20 focus:bg-white outline-none transition-all"
                ...
                <div className="flex flex-col gap-2 md:col-span-2">
                  <label
                    className="font-bold text-sm ml-2 text-foreground/70 uppercase tracking-wider">Cellulare*</label>
                ...
                <div className="flex flex-col gap-2 md:col-span-2">
                  <label className="font-bold text-sm ml-2 text-foreground/70 uppercase tracking-wider">Come possiamo
                    aiutarti?*</label>
                  <textarea name="description" rows={4} required
                            className="bg-muted/50 p-4 rounded-2xl border-2 border-transparent focus:border-primary/20 focus:bg-white outline-none transition-all resize-none"
                            placeholder="Descrivici la tua idea..."></textarea>
                </div>

                <div className="md:col-span-2 py-2">
                  <label className="group flex items-start gap-3 text-sm text-foreground/80 cursor-pointer">
                    <input type="checkbox" required
                           className=" w-5 h-5 rounded-lg border-muted accent-primary cursor-pointer" />
                ...
                <div className="md:col-span-2 pt-4">
                  <button type="submit"
                          className="cursor-pointer w-full py-5 rounded-[1.5rem] bg-brand-pink text-white font-black text-xl  transition-bounce flex items-center justify-center gap-3 group">
                    <Send
